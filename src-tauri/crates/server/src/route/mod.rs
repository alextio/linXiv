...
//!   - serializes domain structs with `serde_json::to_value`,
//!   - emits the exact response envelope — key names AND order (`preserve_order`
//!     keeps struct/`json!` order) — with the status `CoreError::http_status` encodes.
//!
...
impl ApiError {
    ...
    /// Returned when no route arm matches: a plain 404.
...
impl ReqCtx<'_> {
    ...
    /// Deserialize the JSON body into `T`; 422 on a malformed/missing body.
    /// Deserializes from the borrowed `Value` rather than cloning it first —
    /// bodies can be ~100 MB base64 PDF uploads.
...
/// Parse an integer path segment (`{author_id}` etc.); 422 on a non-integer.
...
/// The router proper — `async` so source-backed arms can `.await`; DB-only arms
/// run their `with_conn` closure to completion (no lock held across an await).
...
/// `GET /api/stats` — `service::stats` owns the envelope; all three surfaces emit it.
...
/// `GET /api/categories` — wraps the list in `{"categories": …}` (the CLI emits
/// a bare array; the envelope divergence is deliberate).
...
/// Serialize a domain struct to JSON; an encode failure is a 500.
...
/// `?version=` semantics: absent → None; present must be an int >= 1, else a
/// 422 (not a silent fall-through to latest).
